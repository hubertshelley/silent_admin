...
pub async fn get_sort_list(mut req: Request) -> Result<Res<ListData<SysDictTypeModel>>> {
...
pub async fn get_by_id(mut req: Request) -> Result<Res<SysDictTypeModel>> {
    let req = req.params_parse()?;
    let db = DB.get_or_init(db_conn).await;
...
pub async fn get_all(_req: Request) -> Result<Res<Vec<SysDictTypeModel>>> {
    let db = DB.get_or_init(db_conn).await;

...
use dto::{
    ...
    system::{
        sys_menu::MenuResp,
        sys_role::{
            AddOrCancelAuthRoleReq, DataScopeReq, SysRoleAddReq, SysRoleDeleteReq, SysRoleEditReq,
            SysRoleResp, SysRoleSearchReq, SysRoleStatusReq,
        },
        sys_role_api,
    },
};
use entity::prelude::{SysRole, SysRoleDept};
use entity::{sys_role, sys_role_dept, sys_user_role};
use sea_orm::{
...
pub async fn get_sort_list(
    ...
    if let Some(x) = req.role_ids {
        if !x.is_empty() {
            ...
            s = s.filter(sys_role::Column::Id.is_in(y));
        }
    }
    ...
    if let Some(x) = req.role_name {
        if !x.is_empty() {
            s = s.filter(sys_role::Column::Name.contains(&x));
        }
    }
    if let Some(x) = req.role_key {
        if !x.is_empty() {
            s = s.filter(sys_role::Column::Key.contains(&x));
        }
    }
...
/// add 添加
pub async fn add(db: &DatabaseConnection, req: SysRoleAddReq, _user_id: &str) -> Result<String> {
    //  检查字典类型是否存在
...
pub async fn add_role(txn: &DatabaseTransaction, req: SysRoleAddReq) -> Result<String> {
    ...
    let user = sys_role::ActiveModel {
        id: Set(uid.clone()),
        name: Set(req.role_name),
        key: Set(req.role_key),
        sort: Set(req.list_order),
        data_scope: Set(Some(req.data_scope.unwrap_or_else(|| "3".to_string()))),
...
pub async fn eidt_check_data_is_exist(
    ...
    let c1 = SysRole::find()
        .filter(sys_role::Column::Name.eq(role_name))
        .filter(sys_role::Column::Id.ne(role_id.clone()))
        .count(db)
        .await?;
    let c2 = SysRole::find()
        .filter(sys_role::Column::Name.eq(role_key))
        .filter(sys_role::Column::Id.ne(role_id.clone()))
        .count(db)
        .await?;
...
pub async fn edit(
    ...
    let act = sys_role::ActiveModel {
        name: Set(req.role_name),
        key: Set(req.role_key),
        data_scope: Set(Some(req.data_scope)),
        sort: Set(req.list_order),
        status: Set(req.status),
        remark: Set(req.remark),
        update_by: Set(created_by.to_string()),
        update_time: Set(now),
        ..s_r
    };
    // 更新 //这个两种方式一样 都要多查询一次
...
pub async fn set_status(db: &DatabaseConnection, status_req: SysRoleStatusReq) -> Result<String> {
    ...
    SysRole::update_many()
        ...
        .col_expr(sys_role::Column::UpdateBy, Expr::value(now))
        .filter(sys_role::Column::Id.eq(uid.clone()))
        .exec(&txn)
        .await?;
    txn.commit().await?;
...
pub async fn set_data_scope(db: &DatabaseConnection, req: DataScopeReq) -> Result<String> {
    ...
    let act = sys_role::ActiveModel {
        data_scope: Set(Some(data_scope.clone())),
        update_time: Set(now),
        ..s_r
    };
    act.update(&txn).await?;
...
pub async fn cancel_auth_user(db: &DatabaseConnection, req: AddOrCancelAuthRoleReq) -> Result<()> {
    ...
    sys_user_role::Entity::delete_many()
        .filter(sys_user_role::Column::UserId.is_in(req.clone().user_ids))
        .filter(sys_user_role::Column::RoleId.eq(req.clone().role_id))
        .exec(db)
        .await?;
    txn.commit().await?;
    Ok(())

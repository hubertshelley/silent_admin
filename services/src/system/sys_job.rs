...
use dto::{
    ...
    system::sys_job::{
        SysJobAddReq, SysJobDeleteReq, SysJobEditReq, SysJobSearchReq, SysJobStatusReq, ValidateRes,
    },
};
use entity::prelude::SysJob;
use entity::sys_job;
use sea_orm::{
...
pub async fn get_sort_list(
    ...
    if let Some(x) = search_req.job_name {
        if !x.is_empty() {
            s = s.filter(sys_job::Column::Name.contains(&x));
        }
    }

    if let Some(x) = search_req.job_group {
        if !x.is_empty() {
            s = s.filter(sys_job::Column::Group.contains(&x));
        }
    }
...
{
    let c1 = SysJob::find()
        .filter(sys_job::Column::Name.eq(job_name))
        .count(db)
        .await?;
    let c2 = SysJob::find()
        .filter(sys_job::Column::Id.eq(task_id))
        .count(db)
        .await?;
    Ok(c1 > 0 || c2 > 0)
}
    ...
    C: TransactionTrait + ConnectionTrait,
{
    let c1 = SysJob::find()
        .filter(sys_job::Column::Name.eq(job_name))
        .filter(sys_job::Column::Id.ne(job_id))
        .count(db)
        .await?;
    let c2 = SysJob::find()
        .filter(sys_job::Column::Id.eq(task_id))
    ...
    C: TransactionTrait + ConnectionTrait,
{
    ...
    let add_data = sys_job::ActiveModel {
        ...
        // run_count: Set(0),
        name: Set(Some(req.job_name)),
        // params: Set(req.job_params),
        group: Set(Some(req.job_group)),
        invoke_target: Set(req.invoke_target),
        cron_expression: Set(Some(req.cron_expression)),
        misfire_policy: Set(Some(req.misfire_policy)),
...
pub async fn delete(db: &DatabaseConnection, delete_req: SysJobDeleteReq) -> Result<String> {
    ...
    for job_id in job_ids.clone() {
        if let Some(m) = SysJob::find()
            .filter(sys_job::Column::Id.eq(job_id))
            .one(db)
            .await?
        {
            tasks::delete_job(m.id.parse()?, true)
                .await
                .expect("任务删除失败");
        };
    }
    ...
    let d = SysJob::delete_many()
        .filter(sys_job::Column::Id.is_in(job_ids.clone()))
        .exec(db)
        .await
...
pub async fn edit(db: &DatabaseConnection, req: SysJobEditReq, user_id: String) -> Result<String> {
    ...
    let act = sys_job::ActiveModel {
        ...
        // task_count: Set(req.task_count),
        name: Set(Some(req.job_name)),
        // params: if let Some(x) = req.job_params {
        //     Set(Some(x))
        // } else {
        //     NotSet
        // },
    ...
    tokio::spawn(async move {
        match (
            s_s.status.unwrap_or("0".to_string()).as_str(),
            status.clone().as_str(),
        ) {
            ("0", "1") => {
                tasks::run_circles_task(job_id.clone())
                    .await
                    .expect("任务执行失败");
            }
            ("1", "0") => {
                tasks::delete_job(s_s.clone().id.parse().expect("任务ID错误"), true)
                    .await
                    .expect("任务删除失败");
            }
            ("1", "1") => {
                tracing::info!("任务状态未变化================================================");
                tasks::update_circles_task(job_id.clone())
                    .await
                    .expect("任务更新失败");
...
pub async fn set_status(db: &DatabaseConnection, req: SysJobStatusReq) -> Result<String> {
    ...
    sys_job::Entity::update_many()
        ...
        .filter(sys_job::Column::Id.eq(req.job_id.clone()))
        .exec(db)
        .await?;
    match req.status.clone().as_str() {
        "1" => {
            tasks::run_circles_task(job.clone().id)
                .await
                .expect("任务执行失败");
        }
        "0" => {
            tasks::delete_job(job.clone().id.parse()?, true)
                .await
                .expect("任务删除失败");
        }
...
pub fn validate_cron_str(cron_str: String) -> Result<ValidateRes> {
    ...
    let next_ten: Vec<NaiveDateTime> = schedule
        .upcoming(Local)
        .take(12)
        .map(|x| x.naive_local())
        .collect();

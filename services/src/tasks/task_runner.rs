...
use entity::{sys_job, sys_job_log};
...
pub async fn add_circles_task(t: sys_job::Model) -> Result<()> {
    let task = task_builder::build_task(
        &t.id,
        &t.cron_expression.unwrap_or("".to_string()),
        &t.name.unwrap_or("unknown".to_string()),
        1,
        t.id.try_into().unwrap_or(0),
    );
    match task {
        Ok(x) => {
            init_task_model(t, 1).await;
...
pub async fn update_circles_task(t: sys_job::Model) -> Result<()> {
    let task_count = match 1 {
        x @ 0..=9999 => x, // 防止程序卡死,更新任务时，限制最大任务数
        _ => 9999_i64,
    };
    let task = task_builder::build_task(
        &t.id,
        &t.cron_expression.clone().unwrap_or("".to_string()),
        &t.name.clone().unwrap_or("unknown".to_string()),
        task_count as u64,
        t.id.try_into().unwrap_or(0),
    );
    let remark_update_info_t = format!(
        ...
        &t.cron_expression,
        &t.invoke_target,
        &t.misfire_policy.clone().unwrap_or_default()
    );
    info!("定时任务更新:{}", &remark_update_info_t);
    match task {
        Ok(x) => {
            let t_builder = task_builder::TASK_TIMER.write().await;
            match t_builder.update_task(x) {
                Ok(_) => {
                    ...
                    task_models.entry(t.id).and_modify(|x| {
                        x.model = t.clone();
                        remark = remark.clone()
                            + "    已运行次数:"
...
async fn init_task_model(m: sys_job::Model, task_count: i64) {
    let now = Local::now().naive_local();
    let run_lot = now.and_utc().timestamp();
    let job_end_time =
        get_task_end_time(m.cron_expression.clone().unwrap_or("".to_string()), 1).unwrap();
    let next_time = match get_next_task_run_time(m.cron_expression.unwrap_or("".to_string())) {
        Ok(v) => v.unwrap_or_else(|| Local::now().naive_local()),
        Err(_) => Local::now().naive_local(),
    };
    let mut task_models = TASK_MODELS.lock().await;

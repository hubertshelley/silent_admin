...
pub(crate) async fn up(manager: &SchemaManager<'_>) -> Result<(), DbErr> {
    ...
    if manager.has_table(SysMenu::Table.to_string()).await? {
        manager
    ...
    manager
        .create_table(
            Table::create()
                ...
                .col(
                    ColumnDef::new(SysMenu::OrderNum)
                        .integer()
                        .default(0)
                        .comment("显示顺序"),
                )

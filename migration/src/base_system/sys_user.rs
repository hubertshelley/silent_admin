...
pub(crate) async fn up(manager: &SchemaManager<'_>) -> Result<(), DbErr> {
    ...
    if manager.has_table(SysUser::Table.to_string()).await? {
        manager
    ...
    manager
        .create_table(
            Table::create()
                ...
                .col(
                    ColumnDef::new(SysUser::UserType)
                        .string_len(2)
                        .default("00")
                        .comment("用户类型（00系统用户）"),
                )
                ...
                .col(
                    ColumnDef::new(SysUser::LoginIp)
                        .string_len(36)
                        .default("")
                        .comment("最后登录IP"),
                )
                .col(
                    ColumnDef::new(SysUser::LoginDate)
                        .date_time()
                        .default(Value::String(None))
                        .comment("最后登录时间"),
                )
                .col(
                    ColumnDef::new(SysUser::Remark)
                        .string()
                        .default(Value::String(None))
                        .comment("备注"),
                )
...
pub(crate) async fn init_data(manager: &SchemaManager<'_>) -> Result<(), DbErr> {
    ...
    let insert = Query::insert()
        ...
        .values_panic([
            ...
            "1".into(),
            "".into(),
            "$2a$10$7JB720yubVSZvUI0rEqK/.VqGOZTH.ulu33dHOiBE8ByOhJIrdAu2".into(),
            "admin".into(),
        ])
        .values_panic([
            "2".into(),

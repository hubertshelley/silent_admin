...
pub(crate) async fn up(manager: &SchemaManager<'_>) -> Result<(), DbErr> {
    ...
    if manager.has_table(SysConfig::Table.to_string()).await? {
        manager
...
pub(crate) async fn init_data(manager: &SchemaManager<'_>) -> Result<(), DbErr> {
    ...
    let insert = Query::insert()
        ...
        .values_panic([
            "3".into(),
            "主框架页-侧边栏主题".into(),
            "sys.index.sideTheme".into(),
            "theme-dark".into(),
            "Y".into(),
            "admin".into(),
            "深色主题theme-dark，浅色主题theme-light".into(),
        ])
        .values_panic([
            "4".into(),
            "账号自助-验证码开关".into(),
            ...
            "Y".into(),
            "admin".into(),
            "是否开启验证码功能（true开启，false关闭）".into(),
        ])
        .values_panic([
            "5".into(),
            "账号自助-是否开启用户注册功能".into(),
            "sys.account.registerUser".into(),
            "false".into(),
            "Y".into(),
            "admin".into(),
            "是否开启注册用户功能（true开启，false关闭）".into(),
        ])
        .values_panic([
            "6".into(),
            "用户登录-黑名单列表".into(),
            "sys.login.blackIPList".into(),
            "".into(),
            "Y".into(),
            "admin".into(),
            "设置登录IP黑名单限制，多个匹配项以;分隔，支持匹配（*通配、网段）".into(),
        ])
        .to_owned();

...
pub(crate) async fn up(manager: &SchemaManager<'_>) -> Result<(), DbErr> {
    ...
    if manager.has_table(SysNotice::Table.to_string()).await? {
        manager
            .drop_table(Table::drop().table(SysNotice::Table).to_owned())
            .await?;
    }
    manager
        .create_table(
            Table::create()
                ...
                .append_base_columns()
                .col(
                    ColumnDef::new(SysNotice::Title)
                        .string_len(50)
                        .not_null()
                        .comment("公告标题"),
                )
                .col(
                    ColumnDef::new(SysNotice::Type)
                        .string_len(1)
                        .not_null()
                        .comment("公告类型（1通知 2公告）"),
                )
                .col(
                    ColumnDef::new(SysNotice::Content)
                        .text()
                ...
                .col(
                    ColumnDef::new(SysNotice::Remark)
                        .string()
                        .default(Value::String(None))
                        .comment("备注"),
                )

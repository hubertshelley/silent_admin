...
pub(crate) async fn up(manager: &SchemaManager<'_>) -> Result<(), DbErr> {
    ...
    if manager.has_table(SysJobLog::Table.to_string()).await? {
        manager
    ...
    manager
        .create_table(
            Table::create()
                ...
                .col(
                    ColumnDef::new(SysJobLog::Name)
                        .string_len(64)
                        .not_null()
                        .comment("任务名称"),
                )
                .col(
                    ColumnDef::new(SysJobLog::Group)
                        .string_len(64)
                        .not_null()
                        .comment("任务组名"),
                )
                .col(
                    ColumnDef::new(SysJobLog::InvokeTarget)
                        .string_len(500)
                        .not_null()
                        .comment("调用目标字符串"),
                )
                .col(
                    ColumnDef::new(SysJobLog::JobMessage)
                        .string_len(500)
                        .comment("日志信息"),
                )
                .col(
                ...
                .col(
                    ColumnDef::new(SysJobLog::ExceptionInfo)
                        .string_len(2000)
                        .default("")
                        .comment("异常信息"),
                )
                .col(
                    ColumnDef::new(SysJobLog::CreateTime)
                        .date_time()
                        .comment("创建时间"),
                )
                .comment("定时任务调度日志表")

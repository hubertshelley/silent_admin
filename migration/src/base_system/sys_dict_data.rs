...
pub(crate) async fn up(manager: &SchemaManager<'_>) -> Result<(), DbErr> {
    ...
    if manager.has_table(SysDictData::Table.to_string()).await? {
        manager
    ...
    manager
        .create_table(
            Table::create()
                ...
                .col(
                    ColumnDef::new(SysDictData::Type)
                        .string_len(100)
                        .default("")
                        .comment("字典类型"),
                )
                .col(
                    ColumnDef::new(SysDictData::CssClass)
                        .string_len(100)
                        .default(Value::String(None))
                        .comment("样式属性（其他样式扩展）"),
                )
                .col(
                    ColumnDef::new(SysDictData::ListClass)
                        .string_len(100)
                        .default(Value::String(None))
                        .comment("表格回显样式"),
                )
                .col(
                    ColumnDef::new(SysDictData::IsDefault)
                        .string_len(1)
                        .default("N")
                        .comment("是否默认（Y是 N否）"),
                )
                ...
                .col(
                    ColumnDef::new(SysDictData::Remark)
                        .string()
                        .default(Value::String(None))
                        .comment("备注"),
                )
